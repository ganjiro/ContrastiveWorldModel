...
if __name__ == "__main__":
    ...
    writer_name = "Writers/walker_ME_tecniche/"
    save_path = "Models/walker_ME_tecniche"
    repeats = 5
    ...
    for i in range(repeats):
        ...
        runs.append(manager.test_td3_bc(corr_type=1, aug=2, hyperparameter=0, iterations=500000))
    ...
    for i in range(repeats):
        ...
        manager.train(100) #TODO oCCHIo
        runs.append(manager.test_td3_bc(corr_type=1, aug=4, hyperparameter=0, iterations=500000))
        np.save(os.path.join(writer_name, test_name + '.npy'), np.array(runs))
    runs = np.array(runs)
    np.save(os.path.join(writer_name, test_name + '.npy'), runs)
